...
document.addEventListener('astro:page-load', () => {
  ...
  document.querySelector('n-table[selectable]')?.addEventListener('native:table-select', (e) => {
    selCount++;
    const { allSelected } = /** @type {CustomEvent} */ (e).detail;
    logPrepend(selectionLog, `#${selCount} native:table-select — ${allSelected.length ? allSelected.join(', ') : '(none)'}`);
  });
  ...
  document.querySelector('n-table[reorderable]')?.addEventListener('native:table-reorder', (e) => {
    reorderCount++;
    const { fromIndex, toIndex } = /** @type {CustomEvent} */ (e).detail;
    logPrepend(reorderLog, `#${reorderCount} native:table-reorder — row ${fromIndex} → ${toIndex}`);
  });
  ...
  document.querySelector('n-table[resizable]')?.addEventListener('native:table-resize-end', (e) => {
    resizeCount++;
    const { column, width, allWidths } = /** @type {CustomEvent} */ (e).detail;
    logPrepend(resizeLog, `#${resizeCount} col ${column} → ${Math.round(width)}px  [${allWidths.map(w => Math.round(w)).join(', ')}]`);
  });
