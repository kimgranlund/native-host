...
| **done** | T0154-noodle-controller-trait.md | New trait: NoodleController / noodleable — SVG noodle connections between DOM elements. Noodleable showcase page created with 5 interactive demos + API reference. | native-ui | host |
| **done** | T0155-bare-element-selector-migration.md | Bare HTML element selector migration — `<article>` → `<n-container>`, `<hr>` → `<n-divider>`, layout `<aside>` → `<n-aside>`. All migrated. | native-ui | host |
| **done** | T0156-upgrade-native-ai-1.0.11.md | Upgrade to `@nonoun/native-ai@1.0.15` — SCHEMA pane, Card sub-container pattern (Header/Body/Footer), component map fixes. Package upgraded. | native-ui | host |
| **done** | T0157-a2ui-protocol-page-preset-migration.md | A2UI Protocol page — all 20 presets migrated to Card root + Header/Body/Footer + full-word IDs. Streaming and multi-surface presets updated. Travel presets not added (hidden in package). | native-ui | host |
| **done** | T0158-a2ui-components-page-demo-migration.md | A2UI Components page — all 18 inline demos migrated to Card > Header \| Body \| Footer pattern with full-word IDs. Bus event handlers updated. | native-ui | host |
| **done** | T0159-a2ui-workbench-schema-pane-verification.md | A2UI Workbench — verification-only. SCHEMA pane, 29 updated presets, component map fixes all auto-apply via package upgrade. Verified. | native-ui | host |
| **done** | T0160-store-controller-showcase.md | StoreController — showcase page created with two consumption patterns: direct (imperative) and provider (`<n-controller provides>`). | native-ui | host |
| **open** | T0161-demo-page-api-documentation.md | All 68 demo pages now have structured API Reference sections (attributes, slots, events, CSS, keyboard, accessibility). Host should adopt API tables for public docs. | native-ui | host |
| **open** | T0162-new-container-showcase-pages.md | New showcase pages needed: `n-audio`, `n-picture`, `n-video`, `n-progress`, `.body` container. All CSS-only with demo pages in native-ui. | native-ui | host |
| **done** | T0163-noodleable-demo-color-fix.md | Noodleable demo — flow node colors fixed (raw surface tokens → button-level resolved tokens via `intent` attrs) + style drift cleanup. Applied in noodleable page. | native-ui | host |
| **fixed** | T0164-noodle-port-interaction-improvements.md | NoodleController — port dots need active/dragging/drop-ready states + drop target zone is too small (10px `elementFromPoint`, should use proximity snap ~30px). Fixed in native-ui@0.7.70. | host | native-ui |
| **fixed** | T0165-n-footer-slot-support.md | `n-footer` needs `slot="leading"` / `slot="trailing"` support like `n-header` has — currently just `display: flex; justify-content: flex-end` with no slot handling. Fixed in native-ui@0.7.70. | host | native-ui |
| **open** | T0166-upgrade-native-ui-0.7.70.md | Upgrade to native-ui@0.7.70 — noodle port interaction improvements (proximity snap, drag states) + n-footer slot support. Remove manual footer workarounds. | native-ui | host |
| **done** | T0167-noodleable-page-enhancements.md | Noodleable page — initial connections, alignment guides toggle, present mode, live JSON code editor pane with bidirectional sync, canvas controls overlay, flow-split styling. | host | host |
