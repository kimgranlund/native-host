...
1. **Create the file** in the correct directory. Filename becomes the URL slug: `src/pages/components/ui-tooltip.astro` serves at `/components/ui-tooltip`.
...
7. **If the auto-derived title is wrong**, add a `titleOverrides` entry in `src/data/pages.ts`:
   ...
   const titleOverrides: Record<string, string> = {
     '/components/ui-tooltip': 'Tooltip',   // only if auto-title is wrong
     // ...
   };
   ```
...
~65 pages. Uses `SidebarLayout` with default panels (inspector + chat). Structured with `<h1>` for component name, `<h2>` for sections, `.layout-section` for bordered demo cards, `.layout-row` for flex rows, `.layout-label` for inline size/variant labels, and `.layout-code` for togglable code blocks.
...
<SidebarLayout title="Tooltip">
  <main>
    ...
    <div class="layout-section">
      ...
      <pre class="layout-code"><code set:html={codeExamples.basic} /><n-button class="copy-btn" size="sm" variant="ghost" aria-label="Copy"><n-icon name="copy"></n-icon></n-button></pre>
    </div>
  </main>
</SidebarLayout>
...
| `.layout-label` | Small uppercase label (0.625rem, muted color, fixed 5rem width). Inline size/variant label. |
| `.layout-code` | Code block wrapper. Hidden by default, shown when `[visible]` attribute is set (via code toggle). |
| `.layout-grid` | CSS grid, `auto-fill` columns at 320px min. For multi-column layouts. |
| `.layout-col` | Flex column with 0.5rem gap. Grid child. |
| `.layout-col-label` | Small uppercase label for columns (0.625rem, muted). |
| `.layout-box` | Demo box with border, background, centered text. Generic placeholder. |
| `.layout-card` | Card with center alignment, transitions for hover/press states. Interactive demo target. |
| `.layout-hint` | Small hint text (0.75rem, muted color) below content. |
...
| `.copy-btn` | Copy button floated right inside `.layout-code` blocks. |
...
Code blocks use `.layout-code` wrappers that are hidden by default and toggled visible via the code toggle button in the layout chrome.

Structure:

```html
<pre class="layout-code"><code>&lt;n-button variant="primary"&gt;
...
```ts
const titleOverrides: Record<string, string> = {
  '/components/ui-input-otp': 'Input OTP',    // auto: "Input Otp" (wrong case)
  '/components/ui-kbd': 'Kbd',                 // auto: correct, but explicit for clarity
  '/containers/ui-header': 'Header / Body / Footer',  // page covers 3 elements
...
The key is the URL path without `.astro` extension: `/components/ui-tooltip`, `/blocks/auth-login`, etc.
