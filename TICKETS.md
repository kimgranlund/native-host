# Tickets for @nonoun/native-ui

## TICKET-001: `ui-nav-group` cannot be server-rendered as closed
...
There is no way to server-render a closed nav group. The absence of `open` should mean "closed", but it doesn't — the constructor default overrides the HTML semantics.
